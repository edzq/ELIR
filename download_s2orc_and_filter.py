...
#QI = False # if True, Download CS-only papers with meta-data only
FIELD = "Computer Science" #default: 'Medicine'
og_bash_script = "./dl_s2orc_20200705v1_full_urls_expires_20220311.sh"

# if not QI:
#     papers_to_be_downloaded = pickle.load( open( "papers_to_be_downloaded.p", "rb" ) )
#     p_num = len(papers_to_be_downloaded)
    
def get_links_from_bash(path):
    """
    Parse the given bash scripts for the download urls.
    """
    links = [{} for i in range(100)]

    for line in open(path, 'r').readlines():
        if "wget" in line and "jsonl.gz" in line:
            url = line.split(' \'')[1]
            url = url.replace('\'', '').replace('\n','')
            shard = int(url.split('.jsonl.gz')[0].split('_')[-1])

            if "/full/metadata/metadata_" in url:
                links[shard]["metadata"] = url
            else:
                links[shard]["pdf_parses"] = url
    return links
...
def process_batch(batch: dict):
    ...
    with gzip.open(batch['input_metadata_path'], 'rb') as gz, open(batch['output_metadata_path'], 'wb') as f_out:
        ...
        for line in tqdm(f.readlines()):
            ...
            if mag_field_of_study and FIELD in mag_field_of_study:     # TODO: <<< change this to your filter
                # paper_title = metadata_dict['title'].lower()
                # if QI:
                paper_ids_to_keep.add(paper_id)
                f_out.write(line)
                # elif paper_title in papers_to_be_downloaded:
                #     papers_to_be_downloaded.remove(paper_title)
                #     paper_ids_to_keep.add(paper_id)
                #     f_out.write(line)
...
if __name__ == '__main__':
    ...
    METADATA_OUTPUT_DIR = f'metadata/{FIELD_NAME} sub/'
    PDF_PARSES_INPUT_DIR = 'pdf_parses/raw/'
    PDF_PARSES_OUTPUT_DIR = f'pdf_parses/{FIELD_NAME} sub/'
    ...
    #      "pdf_parses": "https://ai2-s2-s2orc.s3.amazonaws.com/20200705v1/full/pdf_parses/pdf_parses_0.jsonl.gz?AWSAccessKeyId=AKIA5BJLZJPW4OD5EQ2P&Signature=nJuSD314fSU9DujRJ70P09q2iuQ%3D&Expires=1647027264"},  # for shard 0s
    # ]
    download_linkss = get_links_from_bash(og_bash_script)
    ...
    for shard, batch in enumerate(batches):
        print(shard, download_linkss[shard])
        process_batch(batch=batch)
        # if not QI:
        #     print("Number of papers found so far: ", p_num - len(papers_to_be_downloaded))
